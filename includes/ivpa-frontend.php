...
<?php
			...
			<?php
				...
				?>
					...
					<?php
						...
						<?php
							if ( self::$settings['wc_settings_ivpa_single_desc'] == 'ivpa_aftertitle' ) {
								if ( $curr['desc'] !== '' && $curr_is_loop == 'single' ) {
									self::__get_desc( $curr, $curr_is_loop );
								}
								self::__add_price_html_with_wrapper( $curr['attr'], $curr['addprice'], false, $curr_is_loop );
							}
						}
						...
						<?php
							if ( self::$settings['wc_settings_ivpa_single_desc'] == 'ivpa_aftertitle' && $curr['style'] !== 'ivpa_selectbox' ) {
								if ( $curr['desc'] !== '' && $curr_is_loop == 'single' ) {
									self::__get_desc( $curr, $curr_is_loop );
								}
								self::__add_price_html_with_wrapper( $curr['attr'], $curr['addprice'], false, $curr_is_loop );
					...
					<?php
						...
						switch ( $curr['style'] ) {
							...
							case 'ivpa_selectbox' :
								...
								?>
									<span class="ivpa_term ivpa_active<?php echo $curr_add_class; ?>" data-term="<?php echo $b->slug; ?>">
										<?php echo isset( $curr['name'][$b->slug] ) && $curr['name'][$b->slug] !== '' ? $curr['name'][$b->slug] : $b->name; ?> <?php self::add_price_html( $curr['attr'], ( isset( $curr['price'][$b->slug] ) ? $curr['price'][$b->slug] : '' ), false, $curr_is_loop ); ?>
									</span>
								<?php
								...
								?>
									...
									<?php
										if ( ( $curr_is_loop == 'loop' && self::$settings['wc_settings_ivpa_archive_mode'] == 'ivpa_showonly' ) === false ) {
											switch( $curr['style'] ) {
												case 'ivpac_input' :
												?>
													<input class="ivpac-change" type="text" name="<?php echo 'ivpac_' . $k . '[' . $b->slug . ']'; ?>" placeholder="<?php echo isset( $curr['name'][$b->slug] ) && $curr['name'][$b->slug] !== '' ? esc_attr( $curr['name'][$b->slug] ) : esc_attr( $b->name ); ?> "/> 
												<?php
												break;
												...
												case 'ivpac_textarea' :
												?>
													<textarea class="ivpac-change" name="<?php echo 'ivpac_' . $k . '[' . $b->slug . ']'; ?>" placeholder="<?php echo isset( $curr['name'][$b->slug] ) && $curr['name'][$b->slug] !== '' ? esc_attr( $curr['name'][$b->slug] ) : esc_attr( $b->name ); ?>"></textarea> 
												<?php
												break;
									...
									?>
										<span class="ivpa_name">
											<?php echo isset( $curr['name'][$b->slug] ) && $curr['name'][$b->slug] !== '' ? $curr['name'][$b->slug] : $b->name; ?>
										</span>
									<?php
										}
									?>
								...
								<?php
									...
									?>
										...
										<?php
											foreach ( $curr['terms'] as $l => $b ) {
											?>
												<option value="<?php echo $b->slug; ?>" <?php self::add_price_html( $curr['attr'], ( isset( $curr['price'][$b->slug] ) ? $curr['price'][$b->slug] : '' ), true, $curr_is_loop ); ?>><?php echo isset( $curr['name'][$b->slug] ) && $curr['name'][$b->slug] !== '' ? $curr['name'][$b->slug] : $b->name; ?> <?php strip_tags( self::add_price_html( $curr['attr'], ( isset( $curr['price'][$b->slug] ) ? $curr['price'][$b->slug] : '' ), false, $curr_is_loop ) ); ?></option>
											<?php
											}
										?>
									</select> 
		...
		public static function __add_price_html_with_wrapper( $attr, $addprice, $select, $curr_is_loop ) {
			if ( isset( $addprice ) && !empty( $addprice ) && $addprice !== 'false' ) {
			?>
				<div class="ivpa_price">
				<?php
					self::add_price_html( $attr, $addprice, $select, $curr_is_loop );
				?>
				</div>
			<?php
			}
		...
		public static function __get_prices_wrap( $curr_is_loop, $product, $place ) {
			...
			?>
			<strong class="ivpa_title">Cena</strong>
